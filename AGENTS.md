...
Digital Museum · AI 人生档案馆。底层仍是 PRD Phase 0 的 Note → Event Review 本地纵向切片；首页 `/` 已改为“导入记录 → 发现经历（含合并/拆分整理）→ 核对关键内容 → 私人回顾草稿”的价值先行 MVP。当前只接受整理过的 `.md/.txt`，不代表已支持 ChatGPT/Codex/WorkBuddy 原生 Session。`/demo` 是早期策展演示，仅保留视觉方向。
