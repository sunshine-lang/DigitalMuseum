...
npm run test:backend      # 后端 pytest（必须全绿；含四产品同步幂等、适配器行为与历史迁移回归）
...
- `/exhibition` 两阶段：按项目选展准备间（可按月份筛选、展开逐日勾选） → A「现代私人展馆」（2026-09 选定并整合）。沿用一个 Three.js 渲染器，提供浅色空间、白色画框与投影远近层次，无衬线成果标题和深色主按钮；点击远处画框先走近，点击当前画框进入阅读。同一项目一个入口，项目按最近活动排序、内部记录按日期升序，封面由稳定身份 hash 选取。选展/导出仍以原 event ID 为单位；月份过滤只展出及导出当前范围。滚动/按钮/键盘/目录导航，点击展项打开原生 dialog，在日期导航中切换单条档案正文与全部证据；手机版、reduced-motion、WebGL 失败进入 HTML 轻量阅读。Three.js 在进入画廊后动态加载，插画纹理复用，渲染器和事件监听在退出时释放。`public/gallery/` 是通用 AI 纪念插画，必须标为装饰，不能说是事件现场或个性化自动绘图。当前仍无产品内模型调用；可阅读预先保存的候选故事，自动整理和确认保存流程尚未实现。样式集中在 globals.css「A · 现代私人展馆」及其响应式规则；旧 S5 样式仅作阶段遗留。「导出静态展览（HTML）」为无脚本自包含单文件：只含用户勾选展出的事件，证据链细节（锚点、blob 指纹）默认不随导出；产物为午夜档案馆叙事视觉（封面/厅色章头/脊线编年，纯 CSS 无脚本）；导出前有敏感信息扫描（常见密钥、本机路径、邮箱），命中必须人工逐项确认后才落盘——这是 PRD §9 的机械防线，不要移除或改为静默放行。`docs/exhibition-prototype/` 是独立设计原型（含 JavaScript，仅作视觉参照），不是产品代码，勿与正式导出的无脚本契约混淆。PRD v0.3 §11 的真实数据大考复考记录见 `docs/gate/real-data-exam-2026-08-25-v0.3.md`（人工判定字段待用户填写；2026-08-27 已附机器证据附录：26 段单源事件独立复算一致、召回对照、导出产物校验），在用户判定完成前不要把 v0.3 表述为「已通过大考」。
...
- 已保存故事的阅读（2026-09）：`GET /api/v1/retrospectives?project_key=...` 从 `outputs/retrospectives/*/story.json`（可配 `DIGITAL_MUSEUM_RETROSPECTIVES_DIR`）只读加载本地草稿，按真实项目键匹配。只有 `candidate` 可以读取，校验章节出处、逐字引文、日期、文件大小与符号链接；私人正文与来源快照不得硬编码进源码或放入 public。展览详情支持阶段故事/原始记录切换与分章阅读。故事有独立范围，不随当前月份筛选改写，也不随现有静态展览导出；这不是产品内模型生成、编辑核对或“本人确认”保存流程。故事文件独立于档案数据库，现有档案 ZIP 备份与清空操作不包含这些草稿。
- 成果优先经历展品（2026-09 展示实验）：私有 StoryDraft 可选 `exhibit` 字段（标题/概述、独立日期、目标/过程/结果、出处、白名单插画标识），旧草稿兼容。`GET /api/v1/retrospectives/exhibits` 只返回外层摘要目录；画廊仍按项目入口展示，优先显示该项目已保存经历的具体工作与进展，点开再从目标读起，并可切回完整阶段故事或原始记录。`/exhibition?view=gallery` 可用默认已选真实记录直接进入多项目画廊；`/exhibition?exhibit=<story-id>` 保持单项目成果直达，单件/最后一件均有明确提示。无法匹配时回退选展准备间，不按同名项目猜测。未确认经历不进入原始记录导出。本轮仅预存真实样本，不代表自动跨会话归并、成果识别或用户价值验收已完成。纪念插画白名单为 `gathered-pages`（加载 gathered-pages-v2）、`everyday-steps`、`continuous-light`，分别通过本地草稿显式绑定；均为去私密艺术表达，不作历史现场或精确数量图，旧项目封面 hash 池不变。2026-09-13 已保存三个不同真实项目的候选经历样本，仍不代表自动生成或用户价值验收完成。
- 展览项目分组是只读视图，不等同于事件聚合。事件 API 返回 `project_key/project_label/project_path/agent_products`：全部 claim 的来源身份一致才能按项目归组。Codex/pi/dsh 使用已保存的真实 cwd；缺失 source_key/occurrence 的历史记录可从内容哈希校验通过的 Evidence Blob 文档头恢复。Claude 新扫描只在各纳入会话 cwd 一致时追加逐字锚定的 JSON project_path；旧记录不能反解有歧义的转义目录名。归属冲突或无法确定时独立展示，不按标题同名合并，不改写历史档案。
